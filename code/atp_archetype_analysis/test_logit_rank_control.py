...
# Using logistic regression with player_archetype and player_rank
# ---
...
print(f"Saving: {PLOT_PATH}predicted_win_rate.png")
plt.savefig(PLOT_PATH + "predicted_win_rate.png", dpi=300)
...
print(f"Saving: {PLOT_PATH}win_rate_vs_rank.png")
plt.savefig(PLOT_PATH + "win_rate_vs_rank.png")
...
# Full model: player archetype + opponent archetype
matchup_model = smf.logit(
    "won ~ player_archetype + opponent_archetype",
    data=df
).fit(disp=False)

# Reduced model: player archetype only
reduced_matchup = smf.logit(
    "won ~ player_archetype",
    data=df
).fit(disp=False)

# Likelihood-ratio test
lr_stat = 2 * (matchup_model.llf - reduced_matchup.llf)
df_diff = matchup_model.df_model - reduced_matchup.df_model
p_matchup = 1 - chi2.cdf(lr_stat, df_diff)

# Define the order for rows/cols
archetype_order = ["Sprinter", "Balanced", "Endurance"]
present = pd.unique(
    pd.concat([df["player_archetype"], df["opponent_archetype"]], ignore_index=True)
)
archetypes_for_grid = (
    [a for a in archetype_order if a in present] +
    [a for a in present if a not in archetype_order]
)
...
grid = pd.DataFrame(
    ...
    columns=["player_archetype", "opponent_archetype"]
)

grid["pred_prob"] = matchup_model.predict(grid)

# pivot to a matrix with rows = player archetype and cols = opponent archetype
...
# use same 0–1 range as the raw heatmap for visual comparability
...
plt.title(
    "Archetype Matchup Win Probabilities",
    fontsize=16,
    weight="bold"
)
...
p_text = "p < 0.001" if p_matchup < 0.001 else f"p = {p_matchup:.4f}"
significance = "significant" if p_matchup < 0.05 else "not significant"

plt.figtext(
    0.5,
    0.005,
    f"(LR test for opponent archetype: {p_text}, {significance}).",
    ha="center",
    fontsize=11
)

print(f"Saving: {PLOT_PATH}rank_controlled_matchup_heatmap.png")
plt.savefig(PLOT_PATH + "rank_controlled_matchup_heatmap.png", dpi=300)
