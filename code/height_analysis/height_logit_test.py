...
def main():
    ...
    #     values="accuracy",
    # )
    # wide.to_csv(f"{OUTPUT_DIR}/results.csv", index_label="formula")

    # train_data_init = dict()
    # for tier in tiers:
    #     train_data_init[tier] = []
    # for year in data.keys():
    #     # 2024 is the test set.
    #     if year == 2024:
    #         continue
    #     for tier in tiers:
    #         # Early years had no futures for instance.
    #         if tier in data[year]:
    #             df = data[year][tier]
    #             df["year"] = year
    #             train_data_init[tier].append(df)
    # train_data = dict()
    # all = []
    # for tier in tiers:
    #     # Creates a new dataframe, so it will be separate from train_data_init.
    #     train_data[tier] = pd.concat(train_data_init[tier], ignore_index=True)
    #     for frame in train_data_init[tier]:
    #         # Will not affect the rest since concat makes a new frame.
    #         frame["tier"] = tier
    #         all.append(frame)
    # train_data["all"] = pd.concat(all, ignore_index=True)
    # for key in train_data.keys():
    #     train_data[key].to_csv(f"{OUTPUT_DIR}/{key}.csv")
